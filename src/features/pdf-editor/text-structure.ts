...
import { splitWordsWasm } from "@/lib/wasm-core";
...
export function splitRunsIntoWords(text: TextRun[]): TextRun[] {
  const out: TextRun[] = [];
...
export async function splitRunsIntoWordsWasm(text: TextRun[]): Promise<TextRun[]> {
  if (text.length === 0) return [];
  const wasm = await splitWordsWasm(
    text.map((r) => ({
      x: r.x,
      y: r.y,
      width: r.width,
  ...
  if (!wasm) return splitRunsIntoWords(text);
  return wasm.map((w) => ({
    id: createId(),
    x: w.x,
    y: w.y,
...
export async function extractContentRuns(
  ...
  const words =
    opts.engine === "wasm" ? await splitRunsIntoWordsWasm(text) : splitRunsIntoWords(text);
  return { text: words, images };
}